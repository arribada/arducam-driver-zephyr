...
int write_to_photo(struct fs_file_t *file);
...
int camera_init(void)
{
	...
	if (set_mega_resolution(0x1A) > 0) {
		printk("set_mega_resolution failed\n");
	} else {
		printk("set_mega_resolution success\n");
	}
...
int capture_picture(uint8_t *buf, size_t *buf_size)
{
	...
	// fd = fs_open(&file, "/SD:/test_photo.jpg", FS_O_CREATE | FS_O_WRITE);
	// if (fd < 0) {
	// 	printf("Error: Failed to open/create file (%d).\n", fd);
	// 	return fd;
	// }
	...
	// *buf_size = (frame_len * sizeof(uint8_t));
	// buf = malloc(*buf_size);
	...
	// LOG_INF("buffer size %u", *buf_size);
	...
	fd = fs_open(&file, "/SD:/photo.jpg", FS_O_CREATE | FS_O_WRITE);
	if (fd < 0) {
		printf("Error: Failed to open/create file (%d).\n", fd);
		return fd;
	} else {
		printk("/SD:/photo.jpg opened successfully\n");
	}

	while (frame_len > 0) {
		if (!video_get_ctrl(video, VIDEO_CID_ARDUCAM_CAPTURE, vbuf)) {
			// uart_buffer_send(console, vbuf->buffer, vbuf->bytesused);
			// LOG_INF("read fifo :%u. - frame_len %u - buf_index %u", vbuf->bytesused,
			// 	frame_len, buf_index);

			for (buf_index = 0; buf_index < vbuf->bytesused - 1; buf_index++) {
				uint8_t imageData = vbuf->buffer[buf_index];
				uint8_t imageDataNext = vbuf->buffer[buf_index + 1];
			...
			if (start || stop) {
				printk("start writing @ index %d for len %d\n", buf_index,
				       vbuf->bytesused - buf_index);

				ssize_t ret = fs_write(&file, vbuf->buffer[buf_index],
						       vbuf->bytesused - buf_index);
				if (ret < 0) {
					printk("Error: Failed to write to file (%d).\n", ret);
					fs_close(&file);
					return ret;
				}

				printk("first byte %X, second byte %X\n", vbuf->buffer[buf_index],
				       vbuf->buffer[buf_index + 1]);
			...
			// if (imageData == 0xff && imageDataNext == 0xd8) {
			// 	ssize_t ret = fs_write(&file, vbuf->buffer, vbuf->bytesused);
			// 	if (ret < 0) {
	...
	// fd = fs_open(&file, "/SD:/test.txt", FS_O_CREATE | FS_O_WRITE);
	// if (fd < 0) {
	// 	printf("Error: Failed to open/create file (%d).\n", fd);
	// 	return fd;
	// }
...
int write_to_photo(struct fs_file_t *file)
{
	...
	// fd = fs_open(file, "/SD:/test.jpg", FS_O_CREATE | FS_O_WRITE | FS_O_APPEND);
	// if (fd < 0) {
	// 	printf("Error: Failed to open/create file (%d).\n", fd);
	// 	return fd;
	// } else {
	// 	printk("/SD:/test.jpg opened successfully\n");
	// }
...
void serial_cb(const struct device *dev, void *user_data)
{
	...
	while (uart_fifo_read(dev, &c, 1) == 1) {
		printk("uart_msgq_put - %X\n", c);

		if (c == 0x0D && rx_buf_pos > 0) {
			printk("save - %X\n", c);
