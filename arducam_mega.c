...
int arducam_mega_set_auto_focus(uint8_t val)
{
	LOG_INF("Setting autofocus to %d", val);
	camera_write_reg(CAM_REG_AUTO_FOCUS_CONTROL, val);
	camera_wait_idle();
	return 0;
}
...
/* .spi = SPI_DT_SPEC_INST_GET(inst, SPI_LOCK_ON | SPI_HOLD_ON_CS | SPI_OP_MODE_MASTER |
 * SPI_TRANSFER_MSB  | SPI_WORD_SET(8) | SPI_LINES_SINGLE,0), \ */
/* .spi_cfg = {						\ */
/*	.frequency = DT_PROP(DT_NODELABEL(spi0),clock_frequency), \ */
/*	.operation = SPI_LOCK_ON | SPI_HOLD_ON_CS | SPI_OP_MODE_MASTER | SPI_TRANSFER_MSB  |
 * SPI_WORD_SET(8) | SPI_LINES_SINGLE, \ */
